...
const char* codec_name = "libx265";
  ...
  if (ret < 0) {
      HIAI_ENGINE_LOG("[ffmpeg_test] avformat_alloc_output_context2 failed");
      return ret;
  }
  ...
  if (!(encoder_avfc->oformat->flags & AVFMT_NOFILE)) {
      ...
      if (ret < 0) {
          HIAI_ENGINE_LOG("[ffmpeg_test] avio_open2 failed");
          return ret;
      }
  }
  ...
  if (ret < 0) {
    HIAI_ENGINE_LOG("[ffmpeg_test] avcodec_parameters_from_context failed");
    return ret;
  }
  ...
  if (ret < 0) {
    HIAI_ENGINE_LOG("[ffmpeg_test] avformat_new_stream failed");
    return ret;
  }
  ...
  if (ret < 0) {
    HIAI_ENGINE_LOG("[ffmpeg_test] avformat_write_header failed");
    return ret;
  }

  video_frame = av_frame_alloc();
  int frame_buf_size = av_image_get_buffer_size(video_avcc->pix_fmt, video_avcc->width, video_avcc->height, 1);
  HIAI_ENGINE_LOG("[ffmpeg_test] expected frame size %d\n", frame_buf_size);
...
bool FFMPEGContext::IsValid() {
    return valid;
}

void FFMPEGContext::SendFrame(uint8_t* pdata) {
  int ret = 0;
  ...
  AVPacket pkt = {0};
    ...
    if (ret < 0) {
      HIAI_ENGINE_LOG("[FFMPEGContext] avcodec_send_frame failed");
      return;
    }
    ...
    if (ret < 0) {
      HIAI_ENGINE_LOG("[FFMPEGContext] avcodec_receive_packet failed");
      return;
    }
    ...
    if (ret < 0) {
      HIAI_ENGINE_LOG("[FFMPEGContext] av_interleaved_write_frame failed");
      return;
    }
  ...
  HIAI_ENGINE_LOG("[FFMPEGContext] FFMPEGContext::SendFrame!\n");
